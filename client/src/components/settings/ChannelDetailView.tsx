...
import {
  Loader2, CheckCircle2, RefreshCw, Trash2, LogOut, WifiOff, QrCode, ArrowLeft,
} from 'lucide-react';
...
export default function ChannelDetailView({ channel, onUpdate, onBack }: Props) {
  ...
  // Sync effectiveStatus when channel prop changes (e.g., switching channels)
  useEffect(() => {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-start gap-4">
        <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl ${statusConfig.iconBg}`}>
      ...
      {isProvisioning && (
        <div className="flex items-center gap-3 rounded-lg border border-amber-500/30 bg-amber-500/5 p-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">This may take a few minutes. The status will update automatically.</p>
          </div>
        </div>
      )}
      ...
      {/* QR code display */}
      {isAwaitingScan && (
        <div className="flex flex-col items-center gap-4 rounded-lg border border-dashed border-blue-500/30 bg-muted/30 p-6">
          <p className="text-sm font-medium">
            Scan this QR code with your WhatsApp app
          </p>
          {loadingQR ? (
            <Skeleton className="h-[312px] w-[312px] rounded-lg" />
          ) : qrData ? (
            <div className="rounded-lg border bg-white p-4 shadow-sm">
              <QRCodeSVG value={qrData} size={280} />
            </div>
          ) : (
            <div className="flex h-[312px] w-[312px] items-center justify-center rounded-lg border bg-muted/50">
