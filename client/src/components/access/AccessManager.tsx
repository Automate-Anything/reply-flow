...
interface AccessManagerProps {
  ...
  channelMemberIds?: string[]; // For conversation mode: filter "Add override" picker to channel members
  ownerId?: string;
...
export default function AccessManager({
  ...
  // In conversation mode, only show channel members in the "Add override" picker
  const channelMemberSet = channelMemberIds ? new Set(channelMemberIds) : null;

  const availableMembers = teamMembers
    .filter((m) => !existingUserIds.has(m.user_id))
    .filter((m) => !channelMemberSet || channelMemberSet.has(m.user_id))
    .filter(
      (m) =>
        !searchQuery ||
