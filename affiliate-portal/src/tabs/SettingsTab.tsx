...
function SettingsTab({ affiliate, onProfileUpdate }: SettingsTabProps) {
  ...
  useEffect(() => {
    ...
    if (params.get('connect') === 'complete') {
      setConnectMsg('Payout setup complete! Your bank account has been connected.');
      onProfileUpdate();
      // Clean up URL
  ...
  const handleConnectOnboarding = async () => {
    ...
    setConnectMsg('');
    try {
      const res = await connectOnboarding();
      window.location.href = res.url;
    } catch (err: any) {
      setConnectMsg(err.message || 'Failed to start payout setup');
      ...
      <Card title="Payout Setup">
        {affiliate?.bank_account_added ? (
          ...
          <div>
            ...
            <Button onClick={handleConnectOnboarding} disabled={connectLoading} className="mt-4">
              {connectLoading ? 'Redirecting...' : 'Set Up Payouts'}
            </Button>
          </div>
        )}
        {connectMsg && (
          <p className={`text-sm mt-3 ${connectMsg.includes('Failed') ? 'text-[hsl(var(--destructive))]' : 'text-[hsl(var(--success))]'}`}>
            {connectMsg}
          </p>
        )}
      </Card>
