...
const REFRESH_TOKEN_KEY = 'rf_affiliate_refresh_token';
...
localStorage.removeItem('rf_affiliate_access_token');
let accessToken: string | null = null;

// Promise-based refresh queue — concurrent 401s share a single refresh call
...
export async function refreshTokens(): Promise<boolean> {
  ...
  try {
    const res = await fetch(`${API_URL}/api/affiliate/refresh`, {
      method: 'POST',
...
async function request<T = any>(
  ...
  const res = await fetch(`${API_URL}/api/affiliate${endpoint}`, {
    method,
    headers: {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401) {
      ...
      if (refreshed) {
        ...
        const retryRes = await fetch(`${API_URL}/api/affiliate${endpoint}`, {
          method,
          headers: {
            'Content-Type': 'application/json',
...
export const connectOnboarding = () =>
  request<{ url: string }>('/connect-onboarding', 'POST');
export const getConnectStatus = () =>
  request<{ connected: boolean; details_submitted: boolean }>('/connect-status');
